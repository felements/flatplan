...
- `lib/src/storage/` — `PeriodRepository` for YAML read/write, `PeriodStatsWriter` for the AI stats file, and the settings services (storage location, AI stats, macOS security-scoped bookmarks)
- `lib/src/logic/` — Provider-free domain maths: `period_logic.dart` (rollover/bootstrap), `period_extensions.dart` (period end dates, due-date resolution), `period_stats.dart` (`CategoryStats` / `PeriodStats` and the functions deriving them), `allowance_pace.dart` (daily-allowance spending cadence), `period_stats_markdown.dart` (AI stats rendering)
- `lib/src/providers/` — Riverpod providers binding storage to logic: `currentPeriodProvider`, `periodProvider`, `allPeriodsProvider`, `periodStatsProvider`, `periodRepositoryProvider`, `currentPeriodStatsSyncProvider`, and the settings providers
- `lib/src/routing/` — GoRouter with `StatefulShellRoute` (Dashboard + Settings branches)
- `lib/src/views/` — `DashboardView`, `CategoryDetailView`, `CategoryEditorView`, `SettingsView`, `AppShell`
- `lib/src/components/` — `CategoryTile`, `SummaryCard`, `PeriodLoadBanner`, and the `showCategoryDialog` / `showPlannedExpenseDialog` editors
